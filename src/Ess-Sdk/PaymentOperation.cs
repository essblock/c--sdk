...
namespace Ess.Sdk
{
    public class PaymentOperation : Operation
    {
        ...
        {
            Destination = CheckNotNull(destination, "目标账户不能为空");
            Asset = CheckNotNull(asset, "资产不能为空");
            if(amount < 0)
            {
                throw new ArgumentException("数量应该是非负值");
            }
            Amount = amount;
        }
        ...
        public class Builder
        {
            ...
            {
                Destination = CheckNotNull(destination, "目标账户不能是空");
                Asset = CheckNotNull(asset, "资产不能是空");
                if (amount < 0)
                {
                    throw new ArgumentException("数量应该是非负值");
                }
